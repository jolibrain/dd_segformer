...
class MixFFN(nn.Module):
    ...
    def forward(
        self, x, hw_shape: Tuple[int, int], identity: Optional[torch.Tensor] = None
    ):
...
class EfficientMultiheadAttention(MultiheadAttention):
    ...
    def legacy_forward(self, x, hw_shape, identity: Optional[torch.Tensor] = None):
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, hw_shape: Tuple[int, int]):
...
class MixVisionTransformer(nn.Module):
    ...
    def compute_feat(self, x, extract_layer_ids: List[int]):
        outs = []
        feats = []
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i in self.out_indices:
                outs.append(x)
            if extract_layer_ids is not None and i in extract_layer_ids:
                feats.append(x)
